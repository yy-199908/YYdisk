    ...
    ui(new Ui::ButtonGroup)
{
    ...
    m_curBtn=ui->myfile;//一开始显示选中我的文件，并将该图标变红
    ...
    connect(ui->max, &QToolButton::clicked, [=]()
    {
        ...
        if(!fl)
        {
             ui->max->setIcon(QIcon(":/images/Maximize-3.png"));

        }
        else
        {
            ui->max->setIcon(QIcon(":/images/Maximize-1.png"));
        }
        fl = !fl;
        emit maxWindow();
    });
...
void ButtonGroup::paintEvent(QPaintEvent *event)
{
    ...
    QPixmap bk("://images/login_bk.png");
