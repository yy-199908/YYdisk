...
public:
	...
	int des_encode(const char* p, int plen, char**  code);
	int des_decode(const char* code,int clen, char**  decode);

	//加密
	des(const char* key = "abcd1234") { this->key = charToBitset(key); }
	int encoder(const char* p,char* code);
	//解密
	int decoder(const char* code,char *decode);
	//产生16个48位的子密钥,调用encoder，decoder前需要先调用此api
	void generateKeys();
	
private:
	//接受32位数据和48位子密钥，产生一个32位输出
